...
namespace FishingGame.WinForms
{
    public partial class Form1
    {
        ...
        {
            // ---------- Form & perf ----------
            FormBorderStyle = FormBorderStyle.None;   // enlève -, [], X, et le titre
            ControlBox     = false;                    // sécurité : pas de menu système
            Text = "";
           ...
           // ====== Barre titre custom ======
            ...
            // conteneur gauche : icône + titre (ordre garanti)
            var leftStack = new FlowLayoutPanel {
                Dock = DockStyle.Left,
                AutoSize = true,
                WrapContents = false,
                BackColor = Color.Transparent,
                Padding = new Padding(0, 8, 0, 0) // vertical align
            };
            _titleBar.Controls.Add(leftStack);

            // Icône d'app à gauche du titre
            _titleAppIcon = new PictureBox {
                SizeMode = PictureBoxSizeMode.Zoom,
                Width = 25,
                Height = 25,
                Margin = new Padding(0, 2, 8, 0), // espace avant le texte
                BackColor = Color.Transparent
            };
            ...
            // cercle immédiat et automatique si la taille change
            MakeCircle(_btnClose);
            _btnClose.SizeChanged += (_, __) => MakeCircle(_btnClose);
            ...
            try
            {
                // app icon (affiché à gauche du titre)
                var appIco = Path.Combine(AppContext.BaseDirectory, "Assets", "app.ico");
                var appPng = Path.Combine(AppContext.BaseDirectory, "Assets", "app.png");
                ...
                // close icon (png de préférence, sinon glyph)
                var closePng = Path.Combine(AppContext.BaseDirectory, "Assets", "close.png");
                if (File.Exists(closePng))
                {
                    using var raw = Image.FromFile(closePng);
                    _btnClose.Image = new Bitmap(raw, new Size(16, 16)); // bien centré dans le rond
                    _btnClose.Text = "";
                }
                else
                {
            ...
            // drag de fenêtre sur toute la zone gauche
            void HookDrag(Control c) => c.MouseDown += (_, e) => { if (e.Button == MouseButtons.Left) DragWindow(); };
            HookDrag(_titleBar);
            HookDrag(leftStack);
            HookDrag(_titleAppIcon);
            HookDrag(_titleLabel);

            // Icône
            var iconPath = Path.Combine(AppContext.BaseDirectory, "Assets", "app.ico");
            if (File.Exists(iconPath)) Icon = new Icon(iconPath);

            SuspendLayout(); // construire sans relayout intermédiaire
            ...
            // ========== Hôtes joueurs ==========
            Controls.Add(_leftPlayerHost);    // Left
            Controls.Add(_rightPlayerHost);   // Right
            Controls.Add(_bottomPlayerHost);  // Bottom

            // Mains visibles (OverlapHandControl uniquement)
            _leftPlayerHost.Controls.Add(_leftHand);     // Fill
            _rightPlayerHost.Controls.Add(_rightHand);   // Fill
            _bottomPlayerHost.Controls.Add(_bottomHand); // Bottom

            // Supprime toute CardView résiduelle qui pourrait masquer les mains
            ...
            _bottomHand.UseWidthForScale = true;   // main du bas
            _leftHand.UseWidthForScale   = false;  // côtés
            _rightHand.UseWidthForScale  = false;  // côtés

            // s'assure que les mains sont au-dessus des éventuels autres contrôles
            ...
            _btnReset.Click += _btnReset_Click; // utilise StopGameAsync + ResetUI

            // Pas-à-pas : libère les 2 portes (sync + async)
            _btnStep.Click += (_, __) =>
            {
            ...
            UpdateButtonsByState(); // ← état boutons cohérent au tout début
            
            // Keep the title bar truly at the very top (above other Dock=Top controls)
            if (_titleBar != null)
            {
        ...
        private Task<T> UI<T>(Func<T> func)
        {
            if (IsDisposed) return Task.FromCanceled<T>(new CancellationToken(true));
        ...
        // Petit raccourci pour le journal
        private Task LogUI(string line) => UI(() => _journal.AppendText(line));
