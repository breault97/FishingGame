...
namespace FishingGame.WinForms
{
    public partial class Form1
    {
        ...
        // Moteur d’anim "fire-and-forget" côté UI (pas d'attente de BeginInvoke)
        private async Task AnimateImageAsync_FF(Image img, Point start, Point end, int durationMs, CancellationToken ct)
        {
            if (img == null) return;
            ...
            try
            {
                while (true)
                {
                    ...
                    await Task.Delay(15, ct); // cadence ~66 FPS (côté fond)
                }
            }
            finally
            {
        ...
        private (Point start, Point end) GetCenters_DrawToHand_UI(Player current)
        {
            ...
            UI_SYNC(() =>
            {
                ...
                e = CenterOf(GetHandHostFor(current), AnimHost);
                if (e != Point.Empty) e.Offset(-_bottomHand.Width / 3, 0);
            });
            return (s, e);
        }
    }
